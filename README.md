# Sarawak Database Chat

Ask a PostGIS database questions in plain English, see the answer as a table or
on a map, and download it as CSV, JSON, GeoJSON or a shapefile.
...
instead of lon/lat - which is what you want before measuring anything. The
picker next to the Shapefile button offers **EPSG:3376 (GDM2000 East Malaysia
BRSO)** for Sarawak, with the other Malaysian grids below it and legacy datums
labelled as such.

GeoJSON deliberately has no choice: the format is specified as WGS84 lon/lat.

The EPSG codes are read at startup from
...
instead of the Borneo one for this data changes the computed area by 1.9%.
...
| `VISIBLE_DATABASES` | `sarawak basin,subbasin_gis` | which databases appear in the dropdown |
